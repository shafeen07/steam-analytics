...
export async function getTopGames(limit = 50) {
  const res = await fetch(`${API_URL}/api/games/top?limit=${limit}`);
...
export async function searchGames(params: {
  ...
  const res = await fetch(`${API_URL}/api/games/search?${query}`);
  if (!res.ok) throw new Error('Failed to search games');
  return res.json();
}

export async function getGame(appId: number) {
  const res = await fetch(`${API_URL}/api/games/${appId}`, {
...
export async function getPlayerHistory(appId: number, days = 30) {
  const res = await fetch(`${API_URL}/api/players/${appId}/history?days=${days}`, {
    next: { revalidate: 3600 }
  });
  if (!res.ok) return [];
  return res.json();
...
export async function getLastUpdated() {
  const res = await fetch(`${API_URL}/api/players/last-updated`, {
    next: { revalidate: 3600 }
  });
  if (!res.ok) return null;
  return res.json();
